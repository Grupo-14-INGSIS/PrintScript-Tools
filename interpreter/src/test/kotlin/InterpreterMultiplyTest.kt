...
class InterpreterMultiplyTest {
    ...
    fun `multiply two int tokens returns correct result`() {
        ...
        val left = ASTNode(ASTNodeType.NUMBER_LITERAL, "10", Position(1, 1), emptyList())
        val right = ASTNode(ASTNodeType.NUMBER_LITERAL, "4", Position(1, 2), emptyList())
        val parent = ASTNode(ASTNodeType.MULTIPLICATION, "*", Position(1, 0), listOf(left, right))
    ...
    fun `multiply two double tokens returns correct result`() {
        val interpreter = Interpreter()
        val left = ASTNode(ASTNodeType.NUMBER_LITERAL, "10.5", Position(1, 1), emptyList())
    ...
    fun `multiply int and double tokens returns correct result`() {
        val interpreter = Interpreter()
        val left = ASTNode(ASTNodeType.NUMBER_LITERAL, "10", Position(1, 1), emptyList())
        val right = ASTNode(ASTNodeType.NUMBER_LITERAL, "4.2", Position(1, 2), emptyList())
        val parent = ASTNode(ASTNodeType.MULTIPLICATION, "*", Position(1, 0), listOf(left, right))
